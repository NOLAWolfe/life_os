...
const LeakDetector = () => {
    ...
    const smallTransactions = useMemo(() => {
        ...
        const smalls = transactions.filter(t => 
            t.type === 'debit' && 
            t.amount < 30 && 
            t.amount > 0 &&
    ...
    const frequentMerchants = useMemo(() => {
        ...
        transactions.forEach(t => {
            if (t.type !== 'debit' || t.category.includes('Transfers') || t.category.includes('Credit Card Payment')) return;
        ...
        <div className="widget-card h-full flex flex-col">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4 flex-1 overflow-y-auto">
                ...
                <div className="p-4 rounded-lg bg-[var(--bg-primary)] border border-[var(--border-color)]">
                    ...
                    <div className="text-2xl font-bold my-2">{formatCurrency(smallTransactions.total)}</div>
                ...
                <div className="p-4 rounded-lg bg-[var(--bg-primary)] border border-[var(--border-color)]">
                    ...
                    <p className="text-sm text-[var(--text-secondary)]">Top Merchants by Frequency</p>
                    <ul className="mt-2 space-y-2">
                        {frequentMerchants.map((m, idx) => (
                            <li key={idx} className="flex justify-between items-center text-sm border-b border-[var(--border-color)] last:border-0 pb-1">
                ...
                <div className="col-span-1 md:col-span-2 p-4 rounded-lg bg-[var(--bg-primary)] border border-[var(--border-color)]">
                    ...
                    <p className="text-sm text-[var(--text-secondary)]">Add large, irregular bills to see monthly impact</p>
                    
                    <div className="text-xl font-bold text-red-500 my-2">
                        -{formatCurrency(monthlyAmortization)} / mo
                    </div>
                    <p className="text-xs text-[var(--text-secondary)] mb-4">Hidden deduction from your surplus</p>

                    <div className="space-y-2 mb-4">
                        {ghostBills.map(bill => (
                            <div key={bill.id} className="flex justify-between items-center p-2 bg-[var(--bg-secondary)] rounded border border-[var(--border-color)] text-sm">
                                <span>{bill.name} ({formatCurrency(bill.amount)} / {bill.frequency} mo)</span>
                                <button onClick={() => removeGhostBill(bill.id)} className="text-red-500 hover:text-red-400 font-bold px-2">×</button>
                            </div>
                        ))}
                    </div>
                    ...
                    <form onSubmit={addGhostBill} className="flex gap-2">
                        ...
                        <button type="submit" className="bg-blue-600 hover:bg-blue-700 text-white rounded px-3 py-1 text-sm font-medium transition-colors">Add</button>
                    </form>
                </div>
            </div>
        </div>
