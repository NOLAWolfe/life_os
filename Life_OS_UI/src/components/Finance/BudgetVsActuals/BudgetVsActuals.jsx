...
const BudgetVsActuals = () => {
    ...
    const budgetData = useMemo(() => {
        ...
        const targetMonthName = new Date(selectedDate.year, selectedDate.month).toLocaleString('default', { month: 'short' });
        ...
        const findBudgetAmount = (categoryRow) => {
            ...
            for (const pattern of patterns) {
                ...
                if (val !== undefined) return parseFloat(String(val).replace(/[^0-9.-]+/g, "") || 0);
            }
            return 0;
        };
    ...
    if (categories.length === 0) return <div className="widget-card"><p>Please upload 'Categories.csv'.</p></div>;
        ...
        <div className="budget-vs-actuals-container widget-card">
            ...
            <div className="budget-summary flex justify-between items-center bg-[var(--bg-secondary)] p-3 rounded mb-4">
                <div className="text-center">
                    ...
                    <p className="font-bold">{new Intl.NumberFormat('en-US', { style: 'currency', currency: 'USD', maximumFractionDigits: 0 }).format(budgetData.totalBudget)}</p>
                </div>
                <div className="text-center">
                    <p className="text-xs text-[var(--text-secondary)] uppercase">Spent</p>
                    <p className={`font-bold ${budgetData.totalActual > budgetData.totalBudget ? 'text-red-400' : 'text-green-400'}`}>
                        {new Intl.NumberFormat('en-US', { style: 'currency', currency: 'USD', maximumFractionDigits: 0 }).format(budgetData.totalActual)}
                    </p>
                </div>
                <div className="text-center">
                    <p className="text-xs text-[var(--text-secondary)] uppercase">Remaining</p>
                    <p className={`font-bold ${budgetData.totalBudget - budgetData.totalActual < 0 ? 'text-red-400' : 'text-blue-400'}`}>
                        {new Intl.NumberFormat('en-US', { style: 'currency', currency: 'USD', maximumFractionDigits: 0 }).format(budgetData.totalBudget - budgetData.totalActual)}
                    </p>
                </div>
            </div>
                ...
                <table className="w-full text-sm border-collapse">
                    ...
                    <tbody>
                        {budgetData.items.map((item) => (
                            <tr key={item.category} className="border-b border-[var(--border-color)] hover:bg-[var(--bg-secondary)] transition-colors group">
                                ...
                                <td className="text-right py-2 font-mono text-gray-400">
                                    {new Intl.NumberFormat('en-US', { style: 'decimal', minimumFractionDigits: 0, maximumFractionDigits: 0 }).format(item.budget)}
                                </td>
                                <td className="text-right py-2 font-mono font-medium">
                                    {new Intl.NumberFormat('en-US', { style: 'decimal', minimumFractionDigits: 0, maximumFractionDigits: 0 }).format(item.actual)}
                                </td>
                                <td className={`text-right py-2 font-mono pr-2 ${item.difference >= 0 ? 'text-green-400' : 'text-red-400'}`}>
                                    {item.difference > 0 ? '+' : ''}{new Intl.NumberFormat('en-US', { style: 'decimal', minimumFractionDigits: 0, maximumFractionDigits: 0 }).format(item.difference)}
                                </td>
                            </tr>
                        ))}
                    </tbody>
