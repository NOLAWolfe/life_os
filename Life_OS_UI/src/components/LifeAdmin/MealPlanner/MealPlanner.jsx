...
const MealPlanner = () => {
    ...
    const handleRecipeSubmit = async (e) => {
        ...
        const ingredientsArray = newRecipeIngredients.split(',').map(i => i.trim()).filter(i => i);
        const newRecipe = {
            name: newRecipeName,
        ...
        try {
            ...
            if (res.ok) {
                ...
                setRecipes(prev => [...prev, { ...saved, ingredients: JSON.parse(saved.ingredients) }]);
                setNewRecipeName('');
    ...
    const renderPlanGrid = () => (
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-4">
            {DAYS.map(day => {
                ...
                return (
                    <div key={day} className="meal-day-card bg-[var(--bg-secondary)] border border-[var(--border-color)] rounded-lg p-4">
                        <div className="flex justify-between items-center mb-3">
                            <h3 className="font-bold text-[var(--text-primary)]">{day}</h3>
                            <span className="text-xs text-[var(--text-secondary)]">{activeTab === CONSUMERS.USER ? '🏋️' : '🩺'}</span>
                        </div>
                            ...
                            {MEAL_TYPES.map(type => {
                                ...
                                return (
                                    <div key={type} className="meal-slot">
                                        <p className="text-xs uppercase text-[var(--text-secondary)] font-bold mb-1">{type}</p>
                                        <select 
                                        ...
                                        >
                                            ...
                                            {recipes
                                                .filter(r => r.consumer === 'All' || r.consumer === activeTab)
                                                .map(r => (
                                                <option key={r.id} value={r.id}>{r.name}</option>
                                            ))}
                                            <option value="clear" className="text-red-400">(Clear Slot)</option>
                                        </select>
                                    </div>
                                );
                            })}
    ...
    const renderGroceryList = () => {
        ...
        return (
            <div className="bg-[var(--bg-secondary)] p-6 rounded-lg border border-[var(--border-color)] max-w-2xl mx-auto">
                ...
                {list.length === 0 ? (
                    <p className="text-[var(--text-secondary)] italic">Plan some meals to generate a list.</p>
                ) : (
                    <ul className="grid grid-cols-1 md:grid-cols-2 gap-2">
                        {list.map(([item, count]) => (
                            <li key={item} className="flex justify-between items-center bg-[var(--bg-primary)] px-3 py-2 rounded border border-[var(--border-color)]">
                                <span className="capitalize">{item}</span>
                                {count > 1 && <span className="text-xs bg-blue-900 text-blue-200 px-2 py-0.5 rounded-full">{count}x</span>}
                            </li>
                        ))}
                    </ul>
                )}
    ...
    if (loading) return <div className="p-8 text-center animate-pulse">Loading Chef's Engine...</div>;

    return (
        <div className="meal-planner-container space-y-6">
            <header className="flex flex-col md:flex-row justify-between items-center mb-6 gap-4">
                <div>
                    <h2 className="text-2xl font-bold text-[var(--accent-color)]">Household Commander</h2>
                    <p className="text-[var(--text-secondary)]">Week of {new Date(weekStart).toLocaleDateString()}</p>
                </div>
            ...
            {activeTab !== 'List' && (
                <div className="mt-8 pt-8 border-t border-[var(--border-color)]">
                    ...
                    <form onSubmit={handleRecipeSubmit} className="flex flex-col md:flex-row gap-4 items-end bg-[var(--bg-secondary)] p-4 rounded-lg border border-[var(--border-color)]">
                        <div className="flex-1 w-full">
                            <label className="text-xs font-bold text-[var(--text-secondary)] uppercase block mb-1">Recipe Name</label>
                            <input 
                        ...
                        <div className="flex-[2] w-full">
                            <label className="text-xs font-bold text-[var(--text-secondary)] uppercase block mb-1">Ingredients (Comma Separated)</label>
                            <input 
                                className="w-full bg-[var(--bg-primary)] border border-[var(--border-color)] rounded px-3 py-2 text-sm"
                        ...
                        <div className="w-32">
                            <button type="submit" className="w-full bg-[var(--accent-color)] hover:brightness-110 text-white py-2 rounded text-sm font-bold">
                                Save
                            </button>
                        </div>
                    </form>
