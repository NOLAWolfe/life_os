...
const LiabilityModal = ({ onAccept }) => (
    <div className="fixed inset-0 bg-black/90 flex items-center justify-center z-50 p-6">
        <div className="bg-gray-900 border border-red-500/50 rounded-xl p-6 max-w-sm w-full shadow-2xl shadow-red-900/20">
            <h2 className="text-2xl font-black text-red-500 mb-4 tracking-tighter uppercase">⚠️ Liability Waiver</h2>
            <p className="text-gray-300 text-sm mb-6 leading-relaxed">
                By entering the <strong>Titan Protocol</strong>, you acknowledge that physical exercise carries risk of injury. 
                You voluntarily assume all risks. Consult a physician before starting. 
                <br/><br/>
                <span className="text-xs text-gray-500">Life.io is not responsible for swole-related wardrobe malfunctions.</span>
            </p>
            <button 
                onClick={onAccept}
    ...
    return (
        <div className="flex items-center gap-2 bg-gray-800/50 rounded-full px-3 py-1 border border-gray-700">
            ...
            {status === 'connected' ? (
                ...
                <button onClick={handleConnect} className="text-xs font-bold text-gray-300 hover:text-white">
...
const MusicWidget = () => (
    <div className="bg-gradient-to-r from-pink-600 to-purple-700 rounded-xl p-4 flex items-center justify-between shadow-lg mb-6">
        <div className="flex items-center gap-3">
            ...
            <div>
                <p className="text-xs font-bold text-white/80 uppercase tracking-wider">Now Playing</p>
                <p className="text-sm font-black text-white">Beast Mode Playlist</p>
            </div>
        </div>
        <div className="flex gap-2">
            <button className="w-8 h-8 flex items-center justify-center bg-white/20 rounded-full text-white hover:bg-white/30">⏮</button>
            <button className="w-8 h-8 flex items-center justify-center bg-white/20 rounded-full text-white hover:bg-white/30">⏯</button>
            <button className="w-8 h-8 flex items-center justify-center bg-white/20 rounded-full text-white hover:bg-white/30">⏭</button>
        </div>
    </div>
...
const WorkoutTracker = () => {
    const [hasWaiver, setHasWaiver] = useState(() => localStorage.getItem('titan_waiver') === 'true');
    ...
    return (
        <div className="max-w-md mx-auto min-h-screen flex flex-col pb-20">
            ...
            <div className="flex justify-between items-center mb-6 pt-2">
                <h2 className="text-2xl font-black italic tracking-tighter text-white">TITAN<span className="text-blue-500">.OS</span></h2>
                <AppleHealthWidget />
            </div>
            ...
            {activeSession ? (
                <div className="flex-1 flex flex-col">
                    ...
                    <div className="text-center mb-8">
                        <p className="text-sm text-gray-500 uppercase font-bold tracking-widest mb-1">Current Session</p>
                ...
                <div className="flex-1">
                        ...
                        <div className="grid grid-cols-2 gap-4">
                            {['PUSH', 'PULL', 'LEGS', 'CARDIO'].map(type => (
                                ...
                                >
                                    ...
                                    <span className="font-black text-xl text-gray-200 group-hover:text-blue-400">{type}</span>
                                </button>
                            ))}
                        </div>
                    </div>
                    ...
                    <div className="bg-gray-900/50 rounded-xl p-4 border border-gray-800">
                        <h3 className="text-xs font-bold text-gray-500 uppercase tracking-widest mb-3">Recent Activity</h3>
                        <div className="space-y-3">
                            {logs.length === 0 && <p className="text-sm text-gray-600 italic">No workouts logged yet. Get moving!</p>}
                            {logs.map(log => (
                                <div key={log.id} className="flex justify-between items-center text-sm border-b border-gray-800/50 pb-2 last:border-0">
                                    <div>
