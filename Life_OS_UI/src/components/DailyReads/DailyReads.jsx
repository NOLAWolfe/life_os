...
const DailyReads = () => {
  ...
  const handleAddRead = async () => {
    ...
    try {
        ...
        if (res.ok) {
            ...
            fetchReads();
            setFormData({ title: "", author: "", description: "", source: "", type: "book", link: "", category: "General" });
            setShowForm(false);
        }
    } catch (error) {
  ...
  return (
    <div className="daily-reads-container widget-card">
      ...
      {showForm && (
        <div className="add-read-form mb-6 p-4 border border-gray-700 rounded bg-input-bg animate-fade-in">
            ...
            <input
                type="text"
                name="author"
                placeholder={formData.type === 'quote' ? "Author / Speaker" : "Author / Instructor"}
            ...
            <textarea
                name="description"
                placeholder={formData.type === 'quote' ? "Quote Content..." : "Short Description / Notes"}
      ...
      {loading ? <p className="text-gray-500 text-center text-sm py-4">Loading library...</p> : (
        <div className="reads-list space-y-4 max-h-[400px] overflow-y-auto pr-2">
            ...
            <div key={index} className="read-item p-4 rounded bg-gray-800/50 border border-gray-700 hover:border-gray-500 transition-colors">
                <div className="flex justify-between items-start">
                    <h3 className="text-lg font-bold text-yellow-500">
                        {read.type === 'quote' ? `"${read.content || read.description}"` : read.title}
                    </h3>
                ...
                <div className="flex justify-between items-center mt-1">
                    ...
                    {read.tags && read.tags.length > 0 && (
                         <span className="text-xs text-blue-400">{read.tags.join(', ')}</span>
                    )}
                </div>

                {read.type !== 'quote' && read.description && (
                    <p className="text-sm text-gray-300 leading-relaxed mt-2">{read.description}</p>
                )}
            ...
            {reads.length === 0 && <p className="text-center text-gray-500 py-4">No active reads. Start something new!</p>}
        </div>
      )}
    </div>
  );
